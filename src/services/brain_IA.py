...
class Brain:
    def __init__(self):
        self.API_KEY = os.getenv("GLOK_API") # Verifique se o nome da chave está correto no .env
    ...
    def encontrar_melhores_momentos(self, transcricao: List[Dict], duracao_video: float) -> Dict:
        if not self.client:
    ...
    def _validar_momentos(self, data: Dict) -> bool:
        return isinstance(data, dict) and 'momentos' in data and len(data['momentos']) > 0
    ...
    def _fallback_momentos(self, transcricao: List[Dict]) -> Dict:
        ...
        return {"momentos": momentos, "total_segundos": 15.0, "analise": "Análise local (fallback)"}
...
if __name__ == "__main__":
    ...
    print(json.dumps(brain.encontrar_melhores_momentos(exemplo, 30.0), indent=2, ensure_ascii=False))
