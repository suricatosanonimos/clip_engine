    ...
    try:
        ...
        if system == "Linux":
            ...
            result = subprocess.run(
                ["dpkg", "-l", tk_pkg], capture_output=True, text=True
            )
...
class FrameApplierGUI:
    ...
    def _create_gui(self):
        ...
        self.status_var = tk.StringVar(
            value="Pronto. Arraste o retângulo para mover. Bordas para redimensionar."
        )
        ...
        style.configure(
            ".", background="#1a1a2e", foreground="white", font=("Arial", 9)
        )
        ...
        tk.Label(
            self.root,
            text="🖼️  CONFIGURAR MOLDURA & APLICAR",
            font=("Arial", 14, "bold"),
            bg="#1a1a2e",
            fg="#00d4ff",
            height=1,
    ...
    def _build_left_column(self, parent):
        ...
        frame_names = (
            [f.name for f in self.all_frames] if self.all_frames else ["(nenhuma)"]
        )
        self.frame_var = tk.StringVar(value=frame_names[0] if self.all_frames else "")

        frame_combo = ttk.Combobox(
            frame_sel, textvariable=self.frame_var, values=frame_names, state="readonly"
        )
        ...
        self.canvas.tag_bind(
            "handle", "<Enter>", lambda e: self.canvas.config(cursor="fleur")
        )
        self.canvas.tag_bind(
            "handle", "<Leave>", lambda e: self.canvas.config(cursor="")
        )
    ...
    def _build_right_column(self, parent):
        ...
        ttk.Button(sel_frame, text="Todos", command=self._select_all).pack(
            side="left", padx=2
        )
        ttk.Button(sel_frame, text="Nenhum", command=self._select_none).pack(
            side="left", padx=2
        )
        ...
        scrollbar = ttk.Scrollbar(
            list_frame, orient="vertical", command=self.video_listbox.yview
        )
    ...
    def _get_handle_under_mouse(self, event) -> Optional[str]:
        ...
        if abs(mx - va["x"]) < margin and abs(my - (va["y"] + va["h"])) < margin:
            return "sw"
        if (
            abs(mx - (va["x"] + va["w"])) < margin
            and abs(my - (va["y"] + va["h"])) < margin
        ):
            return "se"
    ...
    def _on_mouse_move(self, event):
        ...
        if self._drag_mode == "move":
            self.video_area["x"] = max(
                0, min(va["x"] + int(dx), 720 - self.video_area["w"])
            )
            self.video_area["y"] = max(
                0, min(va["y"] + int(dy), 1280 - self.video_area["h"])
            )
    ...
    def _load_frame(self, frame_path: Path):
        ...
        try:
            ...
            self.canvas.create_image(
                0, 0, anchor="nw", image=self.frame_image, tags="frame_bg"
            )
        except ImportError:
...
class FrameApplier:
    ...
    @classmethod
    def from_gui(
        cls, frame_path: str, video_path: Optional[str] = None
    ) -> Optional["FrameApplier"]:
        gui = FrameApplierGUI()
        result = gui.run()
        return (
            cls(frame_path=result["frame_path"], video_area=result["video_area"])
            if result
            else None
        )
    ...
    def apply_to_video(
        ...
        filter_complex = (
            ...
            f"[1:v]scale={va['w']}:{va['h']}:force_original_aspect_ratio=decrease[vid];"
...
def main():
    ...
    applier = FrameApplier(
        frame_path=result["frame_path"], video_area=result["video_area"]
    )
