...
# O .parent.parent.parent sobe 3 níveis (src/services/ -> src/ -> raiz)
# BASE_DIR = Path(__file__).resolve().parent.parent.parent

# Adiciona a raiz ao sys.path para garantir que os imports funcionem
    ...
    def download_and_split(
        ...
        try:
            ...
            if not video_path.exists():
                ...
                mp4s = sorted(
                    self.base_path.glob(f"{sanitized_title}*.mp4"),
                ...
                if mp4s:
                    ...
                    raise RuntimeError(
                        f"Arquivo não encontrado após download: {final_filename}"
                    )

            logger.info(
                f"Download concluído: {video_path.name} "
