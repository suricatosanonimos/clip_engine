    ...
    for i, clip in enumerate(clips, start=1):
        ...
        try:
            resultado = _run_async(
                gen.process_video(str(clip), cor_legenda=cor_legenda)
            )
            if resultado and resultado.exists():
                ...
                finais.append(clip)
                logger.warning(
                    f"[{task_id}] Legenda falhou para {clip.name}, usando original."
                )
        except Exception as e:
            finais.append(clip)
...
def _download_com_retry(url: str, task_id: str) -> dict:
    ...
    for tentativa in range(1, _DOWNLOAD_MAX_RETRIES + 1):
        try:
            logger.info(
                f"[{task_id}] Download tentativa {tentativa}/{_DOWNLOAD_MAX_RETRIES}"
            )
...
def _run_pipeline(
    ...
    try:
        ...
        logger.info(
            f"[{task_id}] ✅ Pipeline concluído com {len(clip_results)} clipes."
        )
