...
class RegisterUser(BaseModel):
    ...
    nome: str = Field(
        ..., min_length=3, max_length=100, description="Nome completo do usuário"
    )
    email: EmailStr = Field(..., description="Email válido do usuário")
    senha: str = Field(
        ..., min_length=6, max_length=50, description="Senha com mínimo de 6 caracteres"
    )
...
class VideoProcessRequest(BaseModel):
    ...
    user_id: Optional[str] = Field(
        None, description="UUID do usuário autenticado (opcional)"
    )
    job_id: Optional[str] = Field(
        None, description="UUID do job — API cria se não vier"
    )
    ...
    cor_legenda: str = Field(
        default="white", description="white | yellow | blue | green"
    )
